...
getcontext().prec = 18 # Ustawienie precyzji dla operacji na Decimal

def acquire_lock_for_order(order_id: str) -> bool:
    """
    Próbuje atomowo utworzyć dokument blokady w Firestore.
    Zwraca True, jeśli blokada została pomyślnie założona.
    Zwraca False, jeśli blokada już istniała (duplikat).
    """
    try:
        db = get_db()
        doc_ref = db.collection(constants.PROCESSED_ORDER_IDS_COLLECTION).document(order_id)
        
        # Używamy transakcji, aby zapewnić atomowość operacji check-and-set
        @firestore.transactional
        def _create_if_not_exists(transaction, doc_ref):
            snapshot = doc_ref.get(transaction=transaction)
            if snapshot.exists:
                return False # Blokada już istnieje
            else:
                transaction.set(doc_ref, {"processed_at": datetime.now(timezone.utc)})
                return True # Pomyślnie założono blokadę
        ...
        # W przypadku błędu, bezpieczniej jest założyć, że nie udało się założyć blokady
        return False

def log_real_trade_result(pnl_data: Dict[str, Any], active_order_data: Dict[str, Any]) -> bool:
    """
    Przetwarza, wzbogaca i zapisuje wynik transakcji do BigQuery.
    ...
    # Krok 1: Sprawdź, czy ten orderId nie był już przetwarzany
    if not acquire_lock_for_order(order_id):
        return False # To duplikat, nie rób nic więcej

    # Krok 2: Wzbogać dane
    alert_id = active_order_data.get('alert_id', 'UNMATCHED_OR_MANUAL')
    logger.info(f"{log_prefix} Rozpoczynam przetwarzanie i zapis (alert_id: {alert_id}).")

    try:
        # Konwersja na Decimal dla precyzyjnych obliczeń
        ...
        # Pobieranie danych z "złotego rekordu"
        ...
        # Krok 3: Przygotuj finalny obiekt do zapisu
        transformed_data = {
            "alert_id": alert_id,
            "order_id": order_id,
            "symbol": symbol,
            ...
            "timestamp_entry": datetime.fromtimestamp(int(pnl_data.get("createdTime")) / 1000, tz=timezone.utc),
            "timestamp_close": datetime.fromtimestamp(int(pnl_data.get("updatedTime")) / 1000, tz=timezone.utc),
            
            # Kluczowe pola analityczne
