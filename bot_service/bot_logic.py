...
def _handle_setups(klines_data: Dict[str, Kline], active_setups: List[DocumentSnapshot]) -> List[Any]:
        ...
        try:
            ...
            if entry_triggered:
                ...
                if closed_result:
                    ...
                    finalization_tasks.append(finalize_trade(fake_trade, closed_result, close_price))
                    try:
                        db = get_db()
...
def _handle_manage_open_trades(klines_data: Dict[str, Kline], open_trades: List[DocumentSnapshot]) -> List[Any]:
    ...
    for trade_doc in open_trades:
        ...
        try:
            ...
            if not latest_kline:
                logger.warning(f"[{trade_id}] Brak danych kline dla symbolu {trade.symbol}. Pomijam.")
                continue
            closed_result, close_price = None, None
            ...
            if closed_result:
                ...
                try:
                    ...
                    logger.info(f"[{trade_id}] Transakcja zamknięcia pozycji zakończona pomyślnie.")
                except Exception as ex:
...
def run_trading_logic():
    ...
    all_async_tasks = immediate_finalization_tasks + closing_tasks
    if all_async_tasks:
        logger.info(f"Uruchamiam {len(all_async_tasks)} zadań finalizacji.")
        try:
            results = asyncio.run(asyncio.gather(*all_async_tasks, return_exceptions=True))
            for result in results:
                if isinstance(result, Exception):
                    logger.error(f"Błąd w zadaniu asynchronicznym: {result}", exc_info=result)
        except Exception as e:
