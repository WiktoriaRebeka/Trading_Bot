...
def is_alert_processed(alert_id: str) -> bool:
    try:
        doc_ref = _get_db().collection(constants.PROCESSED_ALERT_IDS_COLLECTION).document(alert_id)
        doc = doc_ref.get()
        return doc.exists
    except Exception as e:
        logger.error(f"Błąd podczas sprawdzania alertu {alert_id}: {e}", exc_info=True)
        return True
