# /trading_bot/bot_logic.py (WERSJA FINALNA)
...
def run_trading_logic(all_prices: Dict[str, float]):
    """Główna pętla logiki, która monitoruje aktywne setupy ze stanu w Firestore."""
    ...
    for symbol in active_symbols:
        try:
            ...
            if not setup:
                logger.warning(f"[{symbol}] Setup zniknął ze stanu w trakcie pętli. Pomijam.")
                continue
            ...
            if is_position_open:
                ...
                if not position_data:
                    logger.error(f"[{symbol}] Niespójność stanu: pozycja otwarta, ale brak danych. Usuwam setup.")
                    ...
                    if tp_value is not None and not rr_flags.get(rr_key):
                        if (direction == 'long' and current_price >= tp_value) or \
                           (direction == 'short' and current_price <= tp_value):
                            ...
                            flags_to_update[rr_key] = True
                if flags_to_update:
                    state_manager.update_rr_flags(symbol, flags_to_update)
                ...
                if closed_result:
                    ...
                    trade_data = {
                        ...
                        "symbol": symbol, "direction": direction.upper(), "main_result": closed_result,
                        "ob_type": position_data['ob_type'], "risk_amount_usd": risk_amount_usd,
                        "max_profit_achieved_usd": max_profit_achieved_usd, "rr_achieved": rr_achieved,
                        "rr_1_0_win": rr_flags.get("tp_1_0", False), "rr_1_5_win": rr_flags.get("tp_1_5", False),
                        "rr_2_0_win": rr_flags.get("tp_2_0", False), "rr_3_0_win": rr_flags.get("tp_3_0", False),
                        "rr_4_0_win": rr_flags.get("tp_4_0", False), "rr_5_0_win": rr_flags.get("tp_5_0", False)
                    }
                    log_trade_to_bigquery(trade_data)
                    state_manager.remove_setup(symbol)
            
            else: # Logika wejścia w pozycję
                ...
                try:
                    ...
                    logger.error(f"[{symbol}] Błąd pól w 'ob_data': {e}. Usuwam wadliwy setup. Dane: {ob_data}")
                    state_manager.remove_setup(symbol)
                    continue
                ...
                if should_open:
                    ob_type = "New OB" # Zawsze będzie New, bo stary jest usuwany po zamknięciu
                    trade_id = str(uuid.uuid4())
                    logger.info(f"--- [DECYZJA: WEJŚCIE] --- [{symbol}] | {ob_type} | Cena: {current_price} | ID: {trade_id}")
                    
                    trade_details = {
                        "trade_id": trade_id, "timestamp_entry": datetime.now(timezone.utc).isoformat(),
                        "direction": direction, "ob_type": ob_type, "entry_price": current_price,
                        "sl_price": sl, "main_tp_price": tp, "max_profit_price": current_price,
                        "rr_targets": {
            ...
            logger.error(f"KRYTYCZNY BŁĄD podczas przetwarzania symbolu [{symbol}]. Usuwam setup. Błąd: {e}", exc_info=True)
            state_manager.remove_setup(symbol)
            continue